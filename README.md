...
### เซิร์ฟเวอร์ที่ใช้รันจริง

คอนเทนเนอร์ backend รันด้วย **gunicorn + gevent** (ค่าตั้งอยู่ใน `backend/gunicorn.conf.py`)
ไม่ใช่เซิร์ฟเวอร์สำหรับพัฒนาของ Flask คำสั่ง `python run.py` มีไว้ใช้บนเครื่องตัวเองเท่านั้น

สองอย่างนี้ผูกกันอยู่ อย่าเปลี่ยนทีละอย่าง

- ต้องใช้ worker ของ `gevent-websocket` ไม่ใช่ `gevent` เปล่า ๆ ไม่งั้น Socket.IO
  อัปเกรดเป็น WebSocket ไม่ได้ แล้วถอยไปยิง HTTP ทุก 25 วินาทีต่อเครื่อง
- ต้องใช้ `psycopg2` ไม่ใช่ `psycopg3` เพราะ `psycogreen` ทำให้ psycopg2 ยอมคืนคิว
  ระหว่างรอฐานข้อมูลได้ ส่วน psycopg3 แบบ binary ทำไม่ได้ ถ้าใช้ตัวนั้นทุกคำสั่งฐานข้อมูล
  จะบล็อกทั้ง worker แล้วผู้ใช้ทั้งห้องต้องต่อคิวกันทีละคน

ถ้าจะเพิ่มจำนวน worker เกิน 1 ต้องตั้ง `message_queue` ของ Socket.IO ให้ชี้ไปที่ Redis ก่อน
ไม่งั้นข้อความที่ประกาศจาก worker หนึ่งจะไปไม่ถึงคนที่ต่ออยู่กับอีก worker หนึ่ง
